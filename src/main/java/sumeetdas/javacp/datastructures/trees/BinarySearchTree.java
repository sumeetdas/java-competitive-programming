...
public class BinarySearchTree<T extends Comparable<T>> implements ITree<T> {
    ...
    public static <T extends Comparable<T>> T[] getBFS(Node<T> start, int size) {
        final var queue = new ArrayDeque<Node<T>>();
        final var values = (T[]) Array.newInstance(start.id.getClass(), size);
        var count = 0;
        var node = start;
        while (node != null) {
    ...
    public static <T extends Comparable<T>> T[] getDFS(DepthFirstSearchOrder order, Node<T> start, int size) {
        final var added = new HashSet<Node<T>>(2);
        final var nodes = (T[])Array.newInstance(start.id.getClass(), size);
        int index = 0;
        Node<T> node = start;
