...
public class BTree<T extends Comparable<T>> implements ITree<T> {
    ...
    public boolean add(T value) {
        if (root == null) {
            ...
            while (node != null) {
                ...
                // Greater
                var numberOfKeys = node.numberOfKeys();
                var last = numberOfKeys - 1;
                var greater = node.getKey(last);
    ...
    private void split(Node<T> nodeToSplit) {
        ...
        var numberOfKeys = node.numberOfKeys();
        var medianIndex = numberOfKeys / 2;
        var medianValue = node.getKey(medianIndex);

        var left = new Node<T>(null, maxKeySize, maxChildrenSize);
        ...
        private static <T extends Comparable<T>> String getString(Node<T> node, String prefix, boolean isTail) {
            ...
            if (node.children != null) {
                for (int i = 0; i < node.numberOfChildren() - 1; i++) {
                    var obj = node.getChild(i);
                    builder.append(getString(obj, prefix + (isTail ? "    " : "│   "), false));
                }
                if (node.numberOfChildren() >= 1) {
                    var obj = node.getChild(node.numberOfChildren() - 1);
        ...
        private static class BTreeIterator<C extends Comparable<C>> implements java.util.Iterator<C> {
            ...
            public C next() {
                ...
                while (toVisit.size() > 0) {
                    ...
                    for (var i=0; i<n.childrenSize; i++) {
